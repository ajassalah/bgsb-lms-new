...
export function AssignmentCourseOverview({
  ...
  useEffect(() => {
    const close = (event: PointerEvent) => {
      if (!(event.target as HTMLElement).closest("[data-student-action]"))
        setMenu(null);
    };
    ...
    <>
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">
          Review course assignments and enrolled students.
        </p>
      </div>
      <div className="mt-6 flex gap-2 border-b">
        {(["assignments", "students"] as const).map((item) => (
          <button
            key={item}
            onClick={() => setTab(item)}
            className={`border-b-2 px-5 py-3 text-sm font-bold capitalize ${tab === item ? "border-red-500 text-red-600" : "border-transparent text-slate-500"}`}
          >
            {item}
          </button>
        ))}
              ...
              <tbody className="divide-y">
                {students.map((student, index) => (
                  <tr key={student.id}>
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        {student.avatar ? (
                          ...
                          <span className="grid size-10 place-items-center rounded-full bg-slate-100 font-bold">
                    ...
                    <td className="relative p-5" data-student-action>
                      ...
                      {menu === student.id && (
                        <div className="absolute right-5 top-14 z-[100] w-40 rounded-xl border bg-white py-1 shadow-xl">
                          <Link
                            href={`${basePath}/${courseId}/students/${student.id}`}
          ...
          <div className="mt-7 space-y-5">
            {modules.map((module, moduleIndex) => (
              <details
                ...
                key={module.id}
                className={`group rounded-xl border bg-white ${drag === module.id ? "opacity-50" : ""}`}
              >
                <summary className="flex cursor-pointer list-none items-center gap-4 border-b p-5 [&::-webkit-details-marker]:hidden">
                  <span
                    className={
                      readOnly ? "text-slate-400" : "cursor-grab text-slate-400"
                    }
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {a.file_url && (
                            <small className="mt-2 block font-semibold text-red-600">
